...
/**
 * Created by xuye on 18/01/07
 * 自定义圆环，参考：https://www.jianshu.com/p/74971e1d992e
 */
public class SuperCircleView extends View {
    ...
    public SuperCircleView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mMinCircleColor =
                a.getColor(R.styleable.SuperCircleView_circle_color, context.getResources().getColor(R.color.A9));
    ...
    private void drawColorRing(Canvas canvas) {
        ...
        if (mSelect == mSelectRing && mSelectRing != 0 && mSelect != 0) {
            ...
            canvas.drawArc(mRectF, 270, mRingAngleWidth * mSelectRing + mSelectAngle * mSelectRing, false,
                    ringColorPaint);
        }
        ...
        for (int i = 0; i < mSelectRing; i++) {
            canvas.drawArc(mRectF, 270 + (i * mRingAngleWidth + (i) * mSelectAngle), mSelectAngle, false,
                    ringColorPaint);
        }
    }
