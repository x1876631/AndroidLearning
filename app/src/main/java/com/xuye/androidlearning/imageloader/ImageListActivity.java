...
/**
 * Created by xuye on 17/2/8
 * 照片墙页面，使用imageloader加载图片
 */
public class ImageListActivity extends AppCompatActivity {

    private GridView mImageListView;//图片墙
    ...
    private void initView() {
        mImageListView = (GridView) findViewById(R.id.image_list);
        mImageListView.setAdapter(new ImageListAdapter(this, getImageUrlList()));
    }

    private ArrayList<String> getImageUrlList() {
        ArrayList<String> list = new ArrayList<>();
        for (String url : IMAGE_URL_LIST) {
            list.add(url);
        }
        return list;
    }
